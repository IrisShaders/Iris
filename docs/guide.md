...
- Import button ![importbutton](guide-resources/importbutton.png) : Loads shader setting files (.txt format) into the currently enabled shader. The button opens up a file selection prompt, letting you select the file. Useful for loading custom shader settings found online.
...
- Reset button ![resetbutton](guide-resources/resetbutton.png) : Hold Left SHIFT and click this button to restore the default settings for the selected shaders.
