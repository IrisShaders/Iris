...
public class IrisModelCuboid {
    ...
    public IrisModelCuboid(int u, int v, float x1, float y1, float z1, float sizeX, float sizeY, float sizeZ, float extraX, float extraY, float extraZ, boolean mirror, float textureWidth, float textureHeight, Set<Direction> renderDirections) {
        ...
        Vector3f[] previous = new Vector3f[8];

        for (int i = 0; i < 8; i++) {
            vertices[i].div(16.0f);
            shared[i] = new Vector3f(Float.NaN);
            previous[i] = new Vector3f(Float.NaN);
        }
        ...
        if (renderDirections.contains(Direction.DOWN)) {
            sides[2] = new Quad(new Vector3f[] { shared[5], shared[4], shared[0], shared[1] }, new Vector3f[] { previous[5], previous[4], previous[0], previous[1] }, u1, v0, u2, v1, textureWidth, textureHeight, mirror, Direction.DOWN);
        }

        if (renderDirections.contains(Direction.UP)) {
            sides[3] = new Quad(new Vector3f[] { shared[2], shared[3], shared[7], shared[6] }, new Vector3f[] { previous[2], previous[3], previous[7], previous[6] }, u2, v1, u3, v0, textureWidth, textureHeight, mirror, Direction.UP);
        }

        if (renderDirections.contains(Direction.WEST)) {
            sides[1] = new Quad(new Vector3f[] { shared[0], shared[4], shared[7], shared[3] }, new Vector3f[] { previous[0], previous[4], previous[7], previous[3] }, u0, v1, u1, v2, textureWidth, textureHeight, mirror, Direction.WEST);
        }

        if (renderDirections.contains(Direction.NORTH)) {
            sides[4] = new Quad(new Vector3f[] { shared[1], shared[0], shared[3], shared[2] }, new Vector3f[] { previous[1], previous[0], previous[3], previous[2] }, u1, v1, u2, v2, textureWidth, textureHeight, mirror, Direction.NORTH);
        }

        if (renderDirections.contains(Direction.EAST)) {
            sides[0] = new Quad(new Vector3f[] { shared[5], shared[1], shared[2], shared[6] }, new Vector3f[] { previous[5], previous[1], previous[2], previous[6] }, u2, v1, u4, v2, textureWidth, textureHeight, mirror, Direction.EAST);
        }

        if (renderDirections.contains(Direction.SOUTH)) {
            sides[5] = new Quad(new Vector3f[] { shared[4], shared[5], shared[6], shared[7] }, new Vector3f[] { previous[4], previous[5], previous[6], previous[7] }, u4, v1, u5, v2, textureWidth, textureHeight, mirror, Direction.SOUTH);
        }
        this.quads = sides;
    ...
    public static class Quad {
        ...
        public final Vector3f[] prevPositions;
        ...
        public Quad(Vector3f[] positions, Vector3f[] prevPositions, float u1, float v1, float u2, float v2, float textureWidth, float textureHeight, boolean flip, Direction direction) {
            ...
            if (flip) {
                ...
                for (int i = 0; i < len / 2; ++i) {
                    ...
                    var pos2 = prevPositions[i];
                    prevPositions[i] = prevPositions[len - 1 - i];
                    prevPositions[len - 1 - i] = pos2;
