...
public class ExtendedShader extends ShaderInstance implements ShaderInstanceInterface {
	...
	public void iris$createExtraShaders(ResourceProvider factory, String name) {
		factory.getResource(ResourceLocation.tryBuild("minecraft", name + "_geometry.gsh")).ifPresent(geometry -> {
			try {
		...
		factory.getResource(ResourceLocation.tryBuild("minecraft", name + "_tessControl.tcs")).ifPresent(tessControl -> {
			try {
		...
		factory.getResource(ResourceLocation.tryBuild("minecraft", name + "_tessEval.tes")).ifPresent(tessEval -> {
			try {
