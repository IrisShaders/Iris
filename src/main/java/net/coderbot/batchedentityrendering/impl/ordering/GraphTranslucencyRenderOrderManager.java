...
public class GraphTranslucencyRenderOrderManager implements RenderOrderManager {
    ...
    private final EnumMap<TransparencyType, Digraph<RenderType>> layers;

    private boolean inGroup = false;
    private final EnumMap<TransparencyType, RenderType> currentLayers;
    ...
    public void begin(RenderType layer) {
        ...
        Digraph<RenderType> graph = layers.get(type);
        graph.add(layer);

        if (inGroup) {
			RenderType previous = currentLayers.put(type, layer);
    ...
    public Iterable<RenderType> getRenderOrder() {
        ...
        for (Digraph<RenderType> graph : layers.values()) {
