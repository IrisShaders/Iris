...
public class DepthTexture extends GlResource {
	public DepthTexture(int width, int height, GlFramebuffer depthTexture, DepthBufferFormat format) {
		...
		resize(width, height, depthTexture, format);

		GlStateManager._bindTexture(0);
	}

	void resize(int width, int height, GlFramebuffer depthTexture, DepthBufferFormat format) {
		GlStateManager._bindTexture(getGlId());

		if (depthTexture != null) {
			depthTexture.bindAsReadBuffer();
		...
		GL44C.glClearTexImage(getTextureId(), 0, format.getGlType(), format.getGlFormat(), (ByteBuffer) null);
